...
class PlaceDB (object):
    ...
    def flatten_nested_map(self, *args):
        """
        Flatten an array-of-array map to two CSR-like arrays.

        Compatible with both call styles:
        1) self.flatten_nested_map(nested_map)
        2) self.flatten_nested_map(child2parent_map, nested_map)

        In the second style, child2parent_map is only used to determine
        the total number of pins/elements. The nested_map is the map to flatten.
        """

        if len(args) == 1:
            nested_map = args[0]
            num_elements = sum(len(row) for row in nested_map)
        elif len(args) == 2:
            child2parent_map, nested_map = args
            num_elements = len(child2parent_map)
        else:
            raise TypeError(
                "flatten_nested_map expects 1 or 2 arguments after self, "
                f"but got {len(args)}"
            )

        flat_map = np.zeros(num_elements, dtype=np.int32)
        flat_start_map = np.zeros(len(nested_map) + 1, dtype=np.int32)

        count = 0
        for i in range(len(nested_map)):
            row = nested_map[i]
            ...
            row_len = len(row)
            if row_len > 0:
                flat_map[count:count + row_len] = row

            flat_start_map[i] = count
            count += row_len

        # Be robust if num_elements and actual nested-map length differ.
        # This can happen when some skipped/filtered pins exist.
        if count != num_elements:
            flat_map = flat_map[:count]

        flat_start_map[len(nested_map)] = count

        return flat_map, flat_start_map
    ...
    def prepare_ct_mode_before_initialize(self, params):
        """
        Prepare fields that are normally provided by place_io/rawdb,
        but are absent when PlaceDB is directly built by plc_converter.

        This function must NOT overwrite node/net/pin lists converted from PLC.
        It only fills missing derived/control fields required by initialize()
        and DREAMPlace placement operators.
        """
        ...
        # ------------------------------------------------------------------
        # 2. Basic count fields
        # ------------------------------------------------------------------
        # plc_converter does not explicitly create terminal_NIs.
        if self.num_terminal_NIs is None:
            self.num_terminal_NIs = 0

        # This must be set before anyone accesses self.num_nodes.
        # ------------------------------------------------------------------
        # 3. Make converted arrays dtype-consistent.
        #    Do NOT rebuild or overwrite node/net/pin contents.
        # ------------------------------------------------------------------
        self.node_x = np.asarray(self.node_x, dtype=self.dtype)
        self.node_y = np.asarray(self.node_y, dtype=self.dtype)
        self.node_size_x = np.asarray(self.node_size_x, dtype=self.dtype)
        self.node_size_y = np.asarray(self.node_size_y, dtype=self.dtype)

        self.pin_offset_x = np.asarray(self.pin_offset_x, dtype=self.dtype)
        self.pin_offset_y = np.asarray(self.pin_offset_y, dtype=self.dtype)

        self.pin2node_map = np.asarray(self.pin2node_map, dtype=np.int32)
        self.pin2net_map = np.asarray(self.pin2net_map, dtype=np.int32)

        self.flat_node2pin_map = np.asarray(
            self.flat_node2pin_map, dtype=np.int32
        )
        self.flat_node2pin_start_map = np.asarray(
            self.flat_node2pin_start_map, dtype=np.int32
        )

        self.flat_net2pin_map = np.asarray(
            self.flat_net2pin_map, dtype=np.int32
        )
        self.flat_net2pin_start_map = np.asarray(
            self.flat_net2pin_start_map, dtype=np.int32
        )

        self.rows = np.asarray(self.rows, dtype=self.dtype)

        # ------------------------------------------------------------------
        # 4. Object-array maps.
        #    plc_converter already creates these as object arrays, but keep
        #    this robust for list-based inputs.
        # ------------------------------------------------------------------
        if not isinstance(self.node2pin_map, np.ndarray):
            self.node2pin_map = np.array(
                [np.asarray(x, dtype=np.int32) for x in self.node2pin_map],
                dtype=object,
            )

        if not isinstance(self.net2pin_map, np.ndarray):
            self.net2pin_map = np.array(
                [np.asarray(x, dtype=np.int32) for x in self.net2pin_map],
                dtype=object,
            )
        ...
        if self.flat_region_boxes is None:
            self.flat_region_boxes = np.array([], dtype=self.dtype)

        if self.flat_region_boxes_start is None:
            self.flat_region_boxes_start = np.array([0], dtype=np.int32)

        if self.node2fence_region_map is None:
            self.node2fence_region_map = np.array([], dtype=np.int32)

        # ------------------------------------------------------------------
        # 6. rawdb-only fields.
        # ------------------------------------------------------------------
        if self.node2orig_node_map is None:
            self.node2orig_node_map = np.arange(
                self.num_physical_nodes, dtype=np.int32
            )

        # ------------------------------------------------------------------
        # 7. Timing/net extra fields.
        # ------------------------------------------------------------------
        if not hasattr(self, "net_weight_deltas") or self.net_weight_deltas is None:
            self.net_weight_deltas = np.zeros(len(self.net_names), dtype=self.dtype)

        if not hasattr(self, "net_criticality") or self.net_criticality is None:
            self.net_criticality = np.zeros(len(self.net_names), dtype=self.dtype)

        if (
            not hasattr(self, "net_criticality_deltas")
            or self.net_criticality_deltas is None
        ):
            self.net_criticality_deltas = np.zeros(
                len(self.net_names), dtype=self.dtype
            )

        # ------------------------------------------------------------------
        # 8. Pin names.
        # ------------------------------------------------------------------
        if not hasattr(self, "pin_names") or self.pin_names is None:
            self.pin_names = np.array(
                [f"pin_{i}".encode("utf-8") for i in range(len(self.pin2net_map))],
                dtype=np.string_,
            )

        if not hasattr(self, "pin_name2id_map") or self.pin_name2id_map is None:
            self.pin_name2id_map = {
                self.pin_names[i]: i for i in range(len(self.pin_names))
            }

        # ------------------------------------------------------------------
        # 9. Routing fields.
        #    These are used by scale() and routability logs. If missing,
        #    use layout bbox and simple default capacities.
        # ------------------------------------------------------------------
        if self.routing_grid_xl is None:
            self.routing_grid_xl = self.xl
        if self.routing_grid_yl is None:
            self.routing_grid_yl = self.yl
        if self.routing_grid_xh is None:
            self.routing_grid_xh = self.xh
        if self.routing_grid_yh is None:
            self.routing_grid_yh = self.yh

        if self.num_routing_grids_x is None:
            self.num_routing_grids_x = getattr(
                params, "route_num_bins_x", getattr(params, "num_bins_x", 1)
            )

        if self.num_routing_grids_y is None:
            self.num_routing_grids_y = getattr(
                params, "route_num_bins_y", getattr(params, "num_bins_y", 1)
            )

        if self.num_routing_layers is None:
            self.num_routing_layers = 1

        if self.unit_horizontal_capacity is None:
            self.unit_horizontal_capacity = getattr(
                params, "unit_horizontal_capacity", 1.0
            )

        if self.unit_vertical_capacity is None:
            self.unit_vertical_capacity = getattr(
                params, "unit_vertical_capacity", 1.0
            )

        if self.unit_horizontal_capacities is None:
            self.unit_horizontal_capacities = np.array(
                [self.unit_horizontal_capacity], dtype=self.dtype
            )

        if self.unit_vertical_capacities is None:
            self.unit_vertical_capacities = np.array(
                [self.unit_vertical_capacity], dtype=self.dtype
            )

        if self.initial_horizontal_demand_map is None:
            self.initial_horizontal_demand_map = np.zeros(
                (int(self.num_routing_grids_x), int(self.num_routing_grids_y)),
                dtype=self.dtype,
            )

        if self.initial_vertical_demand_map is None:
            self.initial_vertical_demand_map = np.zeros(
                (int(self.num_routing_grids_x), int(self.num_routing_grids_y)),
                dtype=self.dtype,
            )

        if self.max_net_weight is None:
            self.max_net_weight = np.float64(params.max_net_weight)

        # ------------------------------------------------------------------
        # 10. Safety defaults for macro-related fields.
        # ------------------------------------------------------------------
        if self.movable_macro_mask is None:
            self.movable_macro_mask = np.zeros(self.num_movable_nodes, dtype=bool)

        if self.movable_macro_pins is None:
            self.movable_macro_pins = np.zeros(len(self.pin2node_map), dtype=bool)
    def initialize_ct_filler_nodes(self, params):
        """
        Initialize filler nodes for a PlaceDB generated by plc_converter.

        This function assumes node/net/pin lists have already been converted
        from PLC. It does not overwrite existing physical nodes. It only
        computes filler-related fields and appends filler sizes when enabled.
        """

        # ------------------------------------------------------------
        # 1. Basic safety defaults.
        # ------------------------------------------------------------
        if self.num_terminal_NIs is None:
            self.num_terminal_NIs = 0

        if self.total_filler_node_area is None:
            self.total_filler_node_area = 0.0

        # ------------------------------------------------------------
        # 2. Recompute movable/fixed areas from converted PlaceDB arrays.
        # ------------------------------------------------------------
        ...
        if self.total_space_area is None:
            self.total_space_area = max(
                self.area - self.total_fixed_node_area,
                0.0,
            )

        # ------------------------------------------------------------
        # 3. Detect movable macros.
        # ------------------------------------------------------------
        movable_node_area = (
            self.node_size_x[: self.num_movable_nodes]
            * self.node_size_y[: self.num_movable_nodes]
        ...
        if self.num_movable_nodes > 0:
            mean_movable_node_area = (
                self.total_movable_node_area / self.num_movable_nodes
            )
        else:
            mean_movable_node_area = 0.0

        self.movable_macro_mask = np.zeros(self.num_movable_nodes, dtype=bool)

        if self.num_movable_nodes > 0 and mean_movable_node_area > 0:
        ...
        total_movable_macro_area = float(
            movable_node_area[self.movable_macro_mask].sum()
        )
        ...
        total_cell_space_area = (
            self.total_space_area - total_movable_macro_area
        )
        ...
        # ------------------------------------------------------------
        # 4. If filler is disabled, explicitly set filler fields to zero.
        # ------------------------------------------------------------
        if not params.enable_fillers:
            self.total_filler_node_area = 0.0
            self.num_filler_nodes = 0
            return

        # ------------------------------------------------------------
        # 5. Compute filler size.
        # ------------------------------------------------------------
        ...
        if range_lb >= range_ub:
            ...
            filler_size_x = float(
                np.mean(
                    self.node_size_x[
                        node_size_order[range_lb:range_ub]
                    ]
                )
            )

        filler_size_y = float(self.row_height)
        ...
        if total_cell_space_area <= 0:
            total_cell_space_area = placeable_area
        ...
        # ------------------------------------------------------------
        # 6. Append filler sizes.
        #    Note: fillers do not have node_x/node_y here. BasicPlace will
        #    initialize their positions later using num_filler_nodes and sizes.
        # ------------------------------------------------------------
        ...
        logging.info(
            "CT filler: total_filler_node_area = %g, #fillers = %d, "
            "filler size = %gx%g"
            % (
    ...
    def __call__(self, params):
        """
        @brief top API to read / initialize placement database.
        """
        tt = time.time()

        if getattr(params, "circuit_training_mode", False):
            logging.info(
                "circuit_training_mode=True: skip read(params), "
                "use pre-converted PlaceDB from plc_converter."
            )

            self.prepare_ct_mode_before_initialize(params)

            self.initialize_ct_filler_nodes(params)
            ...
            logging.info(
                "initializing pre-converted PlaceDB takes %g seconds"
                % (time.time() - tt)
            )
            return
...
""" % (
    ...
    def apply(self, params, node_x, node_y):
        ...
        In the normal DREAMPlace flow, rawdb is created by place_io.read().
        In the direct PLC -> PlaceDB flow, rawdb is None, so we only update
        the Python PlaceDB arrays and skip place_io.apply().
        """

        # assign scaled solution back to Python PlaceDB
        ...
        # unscale locations for output/debug
        unscaled_node_x, unscaled_node_y = self.unscale_pl(
            params.shift_factor,
            params.scale_factor,
        )

        # Direct PLC flow: no C++ rawdb exists.
        if self.rawdb is None:
            self.node_x_unscaled = unscaled_node_x
            self.node_y_unscaled = unscaled_node_y

            logging.info(
                "rawdb is None; skip place_io.apply() and keep result in Python PlaceDB."
            )
            return

        # Normal LEF/DEF or Bookshelf flow
        place_io.PlaceIOFunction.apply(self.rawdb, unscaled_node_x, unscaled_node_y)
